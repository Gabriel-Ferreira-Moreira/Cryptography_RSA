...
int Is_Prime_WP(const mpz_t n, int reps); // Miller–Rabin primality test
void ExpoM(mpz_t resultado, const mpz_t base, const mpz_t expoente,
           const mpz_t mod);               // Modular Exponentiation
int Is_Prime_GMP(const mpz_t n, int reps); // primality test GMP

static void BM_Is_Prime_ME(benchmark::State &state) {
  mpz_t n;
...
static void BM_Is_Prime_WP(benchmark::State &state) {
  mpz_t n;
  mpz_init_set_str(n, "1296005097246682578520326409", 10); // Número para teste
  for (auto _ : state) {
    Is_Prime_WP(n, 5); // 5 iterações do teste de primalidade
  }
  mpz_clear(n);
}
...
BENCHMARK(BM_Is_Prime_ME);
BENCHMARK(BM_Is_Prime_GMP);
BENCHMARK(BM_Is_Prime_WP);
...
int Is_Prime_WP(const mpz_t n, int reps) 
{
    ...
    s = mpz_scan1(temp, 0);
    mpz_fdiv_q_2exp(d, temp, s);//d=temp/2^s
